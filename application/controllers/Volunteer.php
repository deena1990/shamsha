...
class Volunteer extends CI_Controller
{
        ...
        if( can('add-volunteer') ) {
            if ($this->input->post('insert')) {
                ...
                if (($this->form_validation->run() == true) && ($imgVal) ) {
                    ...
                    $this->email->to($to);
                    $this->email->subject("Welcome to Shamsaha Application !!");
                    $this->email->message("<html><body><h2>User Login Credentails</h2>
                                            <p>Email: $to</p>
                                            <p>Password: $password</p>
                                        </body></html>");
    ...
    {
        if( can('change_password-volunteer') ) {
    ...
    public function setActiveVols(){
        if ($this->input->post('update')){
            // $data['activeVolsCount'] = $this->input->post('activeVolsCount');
            $this->volunteer_model->saveActiveVolsCount($this->input->post('activeVolsCount'));
            $this->session->set_flashdata('msg', "Minimum Active Volunteers Count has been updated successfully !!");
            $base_url = base_url();
            redirect("$base_url" . "volunteer/setActiveVols");
            print_r($data);die;
            // $this->form_validation->set_data($data);
            // $this->form_validation->set_rules('activeVolsCount', 'Name', 'trim|required');
        }
        $data['activeVolsCount'] = $this->volunteer_model->get_ActiveVolsCount();
        $data['site_title'] = 'Admin Dashboard'; //Title
        $data['page_title'] = 'setActiveVols'; //Page Title
        $this->load->view('header', $data);
        $this->load->view('setactivevolunteer', $data);
        $this->load->view('user_footer');
    }
    ...
    public function edit($id)
    {
        if( can('edit-volunteer') ) {
            ...
            if ($this->input->post('update')) {
                ...
                $data['language_known'] = $this->input->post('language');
                $data['profile_pic'] = $this->input->post('profile_pic');
                $data['onduty_status'] = $this->input->post('onduty_status');
                $data['status'] = $this->input->post('status');
